        ...
        return this.each(function () {
            ...
            that.on('mousedown.DDSort touchstart.DDSort', settings.target, function (e) {
                ...
                .on('mouseup.DDSort touchend.DDSort', function () {
                    ...
                    if(settings.jump){
                    	var clickSectHtml = [];
						$this.parent().find(settings.jump.className).each(function(i){
							clickSectHtml.push($(this).prop("outerHTML"));
						});
						if(clickSectHtml.length != 0){
							if($this.hasClass("clickSect")){
								var len = $this.parent().find(settings.jump.className).length-1;
								var p = $this.parent().find(settings.jump.className).last().prop("outerHTML");
								if($this.offset().top >= $this.parent().find(settings.jump.className).eq(len).offset().top){
									$this.parent().find(settings.jump.className).last().remove();
									that.find(settings.jump.className).last().after(p);
								}
							}else{
								$this.parent().find("tr").remove(settings.jump.className);
								$this.parent().prepend(clickSectHtml.join(""))
							}
						}
						var len = $(settings.jump.className).length;
						that.find("tbody tr").find(".num").html("");
						for(var i=0;i<len;i++){
							that.find("tbody tr").eq(i).find(".num").html(i+1);
						}
	                }
